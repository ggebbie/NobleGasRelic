...
for case in cases
    ...
    !isfile(csvinput) && include(scriptsdir("invert_sixvintages_NPACminusSPAC.jl"))
    ...
    if case == "min_trend"
        ...
        plot(ustrip.(collect(values(t̄))),ustrip.(df[:,6]mbar),ribbon=ustrip.(df[:,7]mbar),label=case,xlabel="calendar years",ylabel="SLP anomaly")
    else
